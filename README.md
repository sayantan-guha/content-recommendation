...
**Completion-rate filtering added** (`src/recommender.py`'s `load_audience_model`): pulled real `content_run_length_secs` + `created_at` via PostHog (`cms_v_watch_history_with_content`) for a sample of users, and computed `completion_pct = seconds_watched / content_run_length_secs`. Validated on a 500-user real-timestamp sample (same LOO methodology): dropping watches confirmed below **60% completion** from the CF matrix improved Hit@10 43.9%→52.7% (+8.8pp), Hit@20 56.6%→67.3% (+10.7pp), NDCG@10 0.289→0.379. Wired into production with **partial coverage by design**: `data/user_watch_completion_sample_1100.csv` has real completion data for 1,100 of the 5,000 production users; watches confirmed below the threshold are dropped from the CF similarity matrix, everything else (no completion data available) passes through unfiltered — so this only ever removes rows we're confident about, never silently degrades coverage elsewhere. At this partial (22%) coverage, the production-scale effect is small and within noise (Hit@10 29.6%→30.2%, Hit@20 40.0%→40.4%) — expected, since the strong validated effect requires completion data for the pair being scored, and most production pairs don't have it yet. Full benefit requires pulling the same fields for the remaining ~3,900 users, not yet done.
