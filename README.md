...
**Edge cases in `recommend_for_user`, found by manually tracing real users whose watch history skewed toward eras/genres with little co-viewing data (see `src/eval_recommender.py` isn't the only test harness — some gaps only show up by hand-testing individual real profiles, not aggregate metrics):**

1. **Low watch history** (`MIN_WATCHED_FOR_CF = 3`) — with only 1-2 watched titles, CF's co-occurrence sum is too thin to trust (222 of 1,100 production users are actually in this bucket). Below the threshold, ranking now uses content-similarity instead of CF.
2. **No similar users found** (`CF_SIGNAL_EPSILON`) — a user can clear the watch-count threshold but still have zero real co-viewers for anything they watched (verified directly on a real user: 3 of their 4 watched titles had *zero* other viewers in our sample, and the 4th had only 2, both of whom otherwise watched only 2020s content — CF's ranking in that case is tie-broken noise, not a real signal, silently returning modern titles regardless of the user's actual taste). Now detected by checking `scores.max() <= CF_SIGNAL_EPSILON` and routed to content-similarity ranking instead.
