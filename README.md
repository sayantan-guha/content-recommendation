...
**Era quota added** (`src/recommender.py`'s `apply_era_quota`): prompted by hand-tracing 1980s/90s-skewed viewers, where CF and content-similarity both naturally recommend 2020s-heavy slates because the catalog itself is 2020s-heavy — same-era candidates score fine on their own similarity terms (genre/actor/director/tags), they just never reach the top of a slate dominated by the newer bulk. This is a post-hoc reordering pass, applied after the existing movie/series type quota: it looks at the era mix in the user's watch history (e.g. 5x 2020s / 4x 2010s / 1x 2000s) and swaps the lowest-priority over-represented-era slots in the already-built slate for the best still-unused same-era candidate pulled from the same relevance-ordered list — it never touches CF/content-similarity's own ranking, and never invents an era the user hasn't actually watched. Verified qualitatively on a real user (5x 2020s/4x 2010s/1x 2000s history): slate went from 7/3/0 to 5/4/1, matching their actual mix exactly. Effect on aggregate recall of the underlying ranking is a wash by design (it's a presentation reorder, not a relevance change) — measuring recall specifically within the final presented slate for held-out pre-2020s titles: Hit@10 51.2%→52.4%, Hit@20 62.8%→61.0%.

Boosting CF's own scores by era match *before* ranking (instead of this post-hoc reorder) was also tried and **rejected** — same failure mode as the rejected creator-boost-on-CF experiment: it dilutes CF's real co-viewer signal with a crude era prior. Monotonically worse the harder it's applied — at boost weight 1.0, overall Hit@10 fell 45.6%→43.9%, and Hit@10 on pre-2020s held-out titles specifically collapsed 51.2%→19.7%. Confirms the post-hoc slate-reorder is the right layer for this, not the scoring layer.
