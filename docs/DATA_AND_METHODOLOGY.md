...
### 3.9 Season-continuation, merged into a single recommendation list (updated 2026-07-17)
**Simple:** if someone has started watching Season 1 of a show, they'll likely want Season 2 once it's available — no need to wait until they've finished every episode of the current season. That recommendation is shown as an ordinary entry in the same single list as everything else (a movie or a show, never a bare episode), just placed first since "you're already watching this" is a stronger signal than a popularity guess.
**Technical:** `src/season_continuation.py` builds a `series_id → {season_number → [content_ids sorted by episode]}` index from the same structured fields (3.8). `recommend_next_seasons()` checks whether a user has genuinely watched (passed the >60%-of-runtime per-episode filter) **at least one** episode of a season (`min_episodes_watched=1`, no completion-percentage gate) and hasn't started the next season; if so, it returns the next season as a **series-level** entity (e.g. `"Sampurna Season 2"`, `content_type='series'`) — never a specific episode content_id. `merge_into_single_list()` then combines continuation recommendations with the regular discovery-ranked output into one deduplicated, truncated list — continuation entries first, discovery entries after — rather than exposing them as a separate UI rail. (An earlier version required ≥90% season completion before recommending the next season; that gate was removed on request — "watched the previous season" is now read as "started it," not "finished it.") Justified by real behavior: 66.1% of 5,000 real users who finished Sampurna Season 1 went on to watch Season 2 — removing the completion gate lets that signal fire earlier in a user's viewing rather than only once a season is fully done.

**Known, deliberately-unfixed gap:** this mechanism only triggers at a season *boundary* (`season_number + 1` existing in the index). Single-season, ongoing shows with no season structure (e.g. Atonko: `season_count=1`, 18 sequentially-released episodes, no season 2) never trigger it, and the discovery ranker separately excludes a series entirely once any episode is known — so a user partway through such a show currently gets no recommendation to continue it. A `recommend_next_episode()` function that walks each series' full `(season_number, episode_number)` sequence (not just season transitions) was built, verified against the real Atonko case, and then explicitly reverted at request.
