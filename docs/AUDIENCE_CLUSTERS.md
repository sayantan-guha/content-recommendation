...
Re-tuned the scoring formula against this new, corrected item catalog (quick blend sweep, `src/pipeline_series_structured.py`):
...
**Season-continuation (updated 2026-07-17):** `src/season_continuation.py`'s `recommend_next_seasons()` recommends the next season of a show whenever a user has watched **any** of the current season — the earlier ≥90%-completion gate has been removed per product direction: "watched the previous season" now means *started* it, not necessarily *finished* it. A single genuinely-engaged episode (still passing the >60%-of-runtime per-episode filter) is enough to suggest the next season exists. Justified by the same real behavior as before: 66.1% of 5,000 real users who finished Sampurna Season 1 went on to watch Season 2 — removing the completion gate means that signal now fires much earlier in a user's viewing, not just at the very end of a season.

The recommendation is always **series-level** (e.g. `"Sampurna Season 2"`), never a bare episode — `content_type` is hardcoded to `'movie'`/`'series'` in the series-rollup construction, fixing an earlier labeling bug where it silently inherited `'episode'` from the underlying row. `merge_into_single_list()` folds continuation entries directly into the same ranked output as regular discovery recommendations (continuation entries placed first, then discovery, deduped, truncated to N) — there is no separate "continuation rail" in the UI-facing list.

**Known gap, not yet fixed:** this only fires at a season **boundary** (`season_number + 1` must exist). Single-season, ongoing/weekly shows with no season structure at all (e.g. Atonko — `season_count=1`, 18 sequentially-numbered episodes, no season 2 ever) get no continuation recommendation no matter how many episodes a user has watched, and the discovery ranker separately excludes the whole series once any episode is known — so a user mid-Atonko is invisible to both mechanisms. A `recommend_next_episode()` function that walks the full `(season_number, episode_number)` sequence within a series (not just season boundaries) was built and verified to close this gap, then explicitly reverted at request — it remains a known, deliberately-unfixed gap, not an oversight.
...
Files: `src/pipeline_series_structured.py`, `src/season_continuation.py`, `data/structured_linkage_350ep.csv`, `data/content_features_series_rollup_structured.csv`.
